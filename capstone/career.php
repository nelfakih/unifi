...
if (!$conn) {
    die("Connection failed: " . mysqli_connect_error());
		...
		$category = "career";
		$query = "SELECT * FROM requestedevents WHERE category='$category' AND approved='Yes'";
    	...
    	while ($row = mysqli_fetch_assoc($result)) {
			...
			echo "<center><a onclick='showPopup()'>More Info</a><div id='popup'><h2>Description</h2><p>". $row['description'] ."</p><button onclick='hidePopup()'>Close</button></div>";
			echo "<a href='registercareer.php'>Register</a></form></center></div></div>";
